...
void Attachment_handler_rx::receive_and_save_attachment(const Attachment_info& attachment_info) {
	const auto file_size = receive_attachment_size();
	...
	while (bytes_received < file_size) {
		const int current_buffer_size = std::min(RX_BUFFER_SIZE, file_size - bytes_received);
...
void Attachment_handler_rx::delete_attachments_from_disc() {
	const std::string storage_path = Administrator::instance().params().get_attachment_storage_path();
	for (const auto& attachment : attachment_array_) {
		const std::string file_path = storage_path + attachment.get_file_name();
...
std::ofstream Attachment_handler_rx::create_unique_file(const Attachment_info& attachment_info) {
	const std::string unique_file_name = create_unique_file_name(attachment_info.get_name());
	attachment_array_.emplace_back(attachment_info.get_name(), attachment_info.get_type(), unique_file_name);

	const std::string path = create_attachment_path(unique_file_name);
...
int Attachment_handler_rx::receive_attachment_size() {
	const int file_size = socket_.receive<int>();
	return file_size;
}
